  ...
  describe VersionedDependencies do
    ...
    describe "#dependency_versions" do
      it "returns versions of all dependencies" do
        ...
        gem1.should_receive(:versions)
            .with(default_args.merge({:versions => {}}), &bl)
            .and_return(versions1)

        versions2 = {:dep2 => {:koji => ['2.0.0']}}
        gem2.should_receive(:versions)
            .with(default_args.merge({:versions => versions1}), &bl)
            .and_return(versions2)
